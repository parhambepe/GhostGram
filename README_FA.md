...
> [!IMPORTANT]
> **امنیت اختصاصی مالک**: تمامی کدهای رمزی تنها از طریق پیام‌های ارسالی خود شما (`event.out`) معتبر هستند. همچنین به محض دریافت، **دستور ارسالی شما فوراً حذف می‌شود** و تأیید آن به‌صورت مخفیانه و موقت در «پیام‌های ذخیره‌شده» شما نمایش داده می‌شود (قابل تنظیم با `STEALTH_CONFIRM`).
...
```
      ...
      • 000 (خاموش چت)    • 777 engage off    • 444 all / کل        • 999 (پاکسازی پیام‌ها)
      • 000 all (خاموش کل)• 777 engage off all  (خاموش کل)         • 555 (داشبورد وضعیت)
                                                                   • !مسدود (لیست سیاه)
                                                                   • 888 (نمایش راهنما)
```
...
| :--- | :--- | :---: | :--- |
| `777` | - | چت جاری | **روشن کردن رفیق** با پرسونای پیش‌فرض (لحن خودمانی و طبیعی). |
| `777 <persona>` | - | چت جاری | **روشن کردن رفیق با لحن خاص** (مثلاً `777 lust`، `777 sarcastic`، `777 poetic`). |
| `000` | - | چت جاری | **خاموش کردن رفیق** در این گفتگو. |
| `000 all` | - | سراسری | **خاموش کردن رفیق در تمام چت‌ها** به صورت یکجا. |
| `777 engage` | - | گروه | **فعال‌سازی تعامل خودکار در گروه** (بررسی وضعیت چت هر ۲۰ دقیقه). |
| `777 engage <min>` | - | گروه | **تعامل خودکار با فاصله زمانی دلخواه** (مثلاً `777 engage 35`). |
| `777 engage off` | - | گروه | **خاموش کردن تعامل خودکار** در گروه فعلی. |
| `777 engage off all` | - | سراسری | **خاموش کردن تعامل خودکار در تمام گروه‌ها**. |
| `666` | - | کل پیوی‌ها | **روشن کردن دستیار شخصی** برای تمامی مخاطبان فعلی و آینده در پیوی. |
| `444` | - | چت جاری | **توقف دستیار فقط در این چت** (دستیار برای سایر پیوی‌ها روشن می‌ماند). |
| `444 all` / `444 کل` | - | کل پیوی‌ها | **خاموش کردن دستیار در تمام پیوی‌ها** (هر دو املا پذیرفته می‌شود). |
| `!مسدود` (روی پیام) | - | چت جاری | **افزودن کاربر به لیست سیاه (جدید)**: دستیار دیگر هرگز به این کاربر پاسخ نمی‌دهد. ID هم قبول است (`!مسدود 123456`). |
| `!آزاد` (روی پیام) | - | چت جاری | **حذف کاربر از لیست سیاه (جدید)**. |
| `111` (روی ریپلای) | - | ریپلای | **پاسخ هوشمند**: پاسخ طبیعی به پیامی که ریپلای زده‌اید بر اساس تاریخچه چت. |
| `111 <دستور>` | - | ریپلای | **پاسخ با دستور دلخواه**: (مثلاً `111 بگو فردا بعدازظهر تماس می‌گیرم`). |
| `333` | - | چت جاری | **ریست حافظه**: پاکسازی تاریخچه کوتاه‌مدت و خلاصه بلندمدت این چت. |
| `999` | - | چت جاری | **پاکسازی روح (کامل)**: حذف آنی تمامی پیام‌های ارسالی شما در این چت. |
| `999 <تعداد>` | - | چت جاری | **پاکسازی بخشی از پیام‌ها**: حذف `N` پیام آخر ارسالی شما در این چت. |
| `555` | - | چت جاری | **وضعیت زنده**: نمایش پنل وضعیت فعلی به مدت ۴ ثانیه و سپس حذف خودکار. |
| `888` | - | چت جاری | **راهنما**: نمایش منوی راهنما و لیست کدهای مخفی. |
| `222` یا `222 <ساعت>` | - | گروه/پیوی | **خلاصه گفتگو**: جمع‌بندی خودمونی اتفاقاتی که در ۱۲ ساعت گذشته (یا بازه دلخواه تا ۷۲ ساعت) از دست شما رفته + اشاره به پیام‌هایی که منتظر جواب شما بوده‌اند. |
| `112 <سؤال>` | - | هرجا | **جستجوی وب**: پاسخ به‌روز با جستجوی گوگل (قیمت ارز، آب‌وهوا، اخبار و...). روی یک پیام هم می‌توانید ریپلای کنید تا درباره همان سؤال بگیرد. تایم‌اوت طولانی‌تر برای نتایج دقیق‌تر. |
| `555 <یادآور>` | - | چت جاری | **یادآور هوشمند**: مثلاً `555 ساعت 21 به علی بگو تماس بگیره` یا `555 تا ۲ ساعت دیگه دارو بخور`. یادآورها بعد از ری‌استارت هم حفظ می‌شوند. |

### 🔔 بازخورد مخفی و گزارش خطا

- تأیید همه دستورها به «پیام‌های ذخیره‌شده» خودتان می‌رود و بعد از چند ثانیه خودبه‌خود پاک می‌شود — هیچ ردی در چت مقصد باقی نمی‌ماند.
- اگر موتور هوش مصنوعی یا ارسال پیام خطا بدهد، گزارش دقیق به «پیام‌های ذخیره‌شده» شما ارسال می‌شود؛ دیگر هیچ خطایی بی‌صدا گم نمی‌شود (با `NOTIFY_ERRORS=0` غیرفعال می‌شود).

### 👁️ درک چندرسانه‌ای

وقتی حالت «رفیق» (777) یا «دستیار» (666) فعال باشد، ربات دیگر فقط متن نمی‌فهمد:

- 📷 **عکس** بفرستید → محتوای عکس توسط Gemini تحلیل می‌شود و طبیعی جواب می‌دهد.
- 🎙️ **ویس** بفرستید → صدای شما شنیده و فهمیده می‌شود و به‌صورت متنی (با لحن انسانی) پاسخ می‌گیرید.
- 🎬 **ویدیو نوت و ویدیو** بفرستید → محتوای ویدئو دیده و فهمیده می‌شود (تا ~۱۸ مگابایت).
- 📄 **PDF و فایل متنی** بفرستید → محتوای فایل خوانده و تحلیل می‌شود.

*(یادآورها در فایل `reminders_state.json` ذخیره می‌شوند و با ری‌استارت از بین نمی‌روند.)*
...
# =================================================================
# ☁️ دیپلوی ابری Railway (پیشنهادی برای اجرای ۲۴ ساعته)
# =================================================================
# یک بار روی سیستم خودت اجرا کن:  python3 gen_session_string.py
# خروجی را در متغیر SESSION_STRING در Railway بگذار.
SESSION_STRING=
# مسیر Volume وصل‌شده به سرویس (state ها ماندگار می‌شوند)
DATA_DIR=/app/data

# =================================================================
# 🔔 بازخورد مخفی و گزارش خطا (اختیاری — پیش‌فرض فعال است)
# =================================================================
STEALTH_CONFIRM=1
...
# =================================================================
# ⏱ تایم‌اوت موتور جمینای (جستجوی وب به زمان بیشتری نیاز دارد)
# =================================================================
GEMINI_TIMEOUT=15
SEARCH_TIMEOUT=45
...
### ☁️ چک‌لیست دیپلوی روی Railway

1. یک **Volume** بسازید و به سرویس GhostGram در مسیر `/app/data` وصل کنید (بدون این، حافظه و یادآورها با هر ری‌استارت پاک می‌شوند).
2. متغیرهای `API_ID`، `API_HASH`، `GEMINI_API_KEYS`، `OWNER_ID` را تنظیم کنید.
3. روی سیستم خودتان `python3 gen_session_string.py` را اجرا کنید و خروجی را در متغیر `SESSION_STRING` بگذارید — بدون این، هر بار که کانتینر ری‌استارت شود تلگرام دوباره کد لاگین می‌خواهد.
